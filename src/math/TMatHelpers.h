...
template<typename MATRIX>
TVec3<typename MATRIX::value_type> extractScale(const MATRIX& mat) {
    typedef typename MATRIX::value_type T;

    TVec3<T> scale;
    scale.x = (T)std::sqrt(mat[0][0] * mat[0][0] + mat[0][1] * mat[0][1] + mat[0][2] * mat[0][2]);
    scale.y = (T)std::sqrt(mat[1][0] * mat[1][0] + mat[1][1] * mat[1][1] + mat[1][2] * mat[1][2]);
    scale.z = (T)std::sqrt(mat[2][0] * mat[2][0] + mat[2][1] * mat[2][1] + mat[2][2] * mat[2][2]);
...
public:
    ...
    TVec3 <T> toScale() const {
        return matrix::extractScale(static_cast<const BASE<T>&>(*this));
    }
};
